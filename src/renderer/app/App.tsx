...
import { computeRegionPlacement, projectGeometry, projectSpawnPoints, toAbsolutePlacement } from "../../composition/coordinate-transform";
...
export function App() {
  ...
  const [geometryMode, setGeometryMode] = useState<GeometryMode>("grid"); const [geometryDensity, setGeometryDensity] = useState(8); const [showPieceBorders, setShowPieceBorders] = useState(true);
  ...
  useEffect(() => { referenceFrameRef.current = referenceFrame; geometryRef.current = geometry; mappingRef.current = mapping; timingRef.current = timingSettings; expressionRef.current = expressionSettings; showPieceBordersRef.current = showPieceBorders; }, [referenceFrame, geometry, mapping, timingSettings, expressionSettings, showPieceBorders]);
  ...
  function projectedMappingFor(): MidiMappingResult | undefined {
    ...
    const spawnPlacement = referenceImg?.width && referenceImg.height ? computeRegionPlacement(referenceImg.width, referenceImg.height, DEFAULT_LAYOUT.pianoRegion) : { offsetX: 0, offsetY: 0, scale: 1 };
  ...
  useEffect(() => {
    ...
    return () => { disposed = true; app.renderer?.off("resize", onResize); app.ticker?.remove(tick); if (initialized) app.destroy(true); };
  }, []);
  ...
  function removeReferenceFrame() { setReferenceFrame(undefined); setReferenceStatus("empty"); setReferenceError(""); setCalibration(undefined); setMapping(undefined); if (pixi.current) { pixi.current.stage.removeChildren(); spriteRef.current = null; overlay.current = new Container(); pixi.current.stage.addChild(overlay.current); } }
  ...
  function placeManualPoint(midiNote: number, event: React.MouseEvent<HTMLDivElement>) {
    ...
    const rect = host.current.getBoundingClientRect();
    const projection = computeViewProjection(calibration, pixi.current.screen.width, pixi.current.screen.height);
    const source = toSourcePoint({ x: event.clientX - rect.left, y: event.clientY - rect.top }, projection);
    const point = { x: Math.max(0, Math.min(calibration.sourceWidth, source.x)), y: Math.max(0, Math.min(calibration.sourceHeight, source.y)) };
