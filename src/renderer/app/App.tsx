...
import { MidiRecorder } from "../../midi/midi-recorder";
...
import { createPianoLayout } from "../../keyboard/piano-layout";
...
export function App() {
  ...
  const [recordingState, setRecordingState] = useState<{ isRecording: boolean; isPaused: boolean; durationMs: number; noteCount: number; midiInputs: Array<{ id: string; name: string; manufacturer: string }> }>({ isRecording: false, isPaused: false, durationMs: 0, noteCount: 0, midiInputs: [] });
  ...
  useEffect(() => {
    ...
    rec.onChangeCallback = (state) => {
      setRecordingState((s) => ({ ...s, isRecording: state.isRecording, isPaused: state.isPaused, durationMs: state.durationMs, noteCount: state.notes.length }));
    };
    return () => { rec.dispose(); };
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    panel.innerHTML = `
      ...
      <label class="toggle-row"><span>Glow Effect</span><input data-fx="glowEnabled" type="checkbox"></label>
  ...
  async function importPuzzleArtwork() { if (hasElectronApi) { setPuzzleArtworkStatus("loading"); try { const file = await window.pianoPuzzle?.chooseAsset("image"); if (!file) { setPuzzleArtworkStatus("empty"); return; } await acceptPuzzleArtwork(file.filePath, file.fileName, file.mimeType, `data:${file.mimeType};base64,${file.dataBase64}`, file.fileSize); } catch (caught) { setPuzzleArtworkStatus("error"); setPuzzleArtworkError(caught instanceof Error ? caught.message : "بارگذاری تصویر پازل ناموفق بود."); } } else { artworkImageInputRef.current?.click(); } }
  function handleArtworkFileChange(e: React.ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (!file) return; setPuzzleArtworkStatus("loading"); const reader = new FileReader(); reader.onload = () => void acceptPuzzleArtwork("", file.name, file.type, String(reader.result), file.size); reader.onerror = () => { setPuzzleArtworkStatus("error"); setPuzzleArtworkError("خواندن تصویر ناموفق بود."); }; reader.readAsDataURL(file); e.target.value = ""; }
  ...
  async function importMidi() { if (hasElectronApi) { setMidiStatus("loading"); setMappingError(""); try { const file = await window.pianoPuzzle?.chooseAsset("midi"); if (!file) { setMidiStatus("empty"); return; } const bytes = Uint8Array.from(atob(file.dataBase64), (char) => char.charCodeAt(0)); const parsed = parseMidi(bytes.buffer, file.fileName, project.midiSettings.selectedTrackIndices, project.midiSettings.chordTolerance); setMidi(parsed); setMidiAsset({ id: crypto.randomUUID(), type: "midi", ...file, duration: parsed.duration, importedAt: new Date().toISOString(), status: "loaded", role: "midi" }); setMidiStatus("loaded"); setMapping(undefined); } catch (caught) { setMidiStatus("error"); setMappingError(caught instanceof Error ? caught.message : "بارگذاری MIDI ناموفق بود."); } } else { midiFileInputRef.current?.click(); } }
  function handleMidiFileChange(e: React.ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (!file) return; setMidiStatus("loading"); setMappingError(""); const reader = new FileReader(); reader.onload = () => { try { const dataUrl = String(reader.result); const base64 = dataUrl.split(",")[1] ?? ""; const bytes = Uint8Array.from(atob(base64), (char) => char.charCodeAt(0)); const parsed = parseMidi(bytes.buffer, file.name, project.midiSettings.selectedTrackIndices, project.midiSettings.chordTolerance); setMidi(parsed); setMidiAsset({ id: crypto.randomUUID(), type: "midi", filePath: "", fileName: file.name, fileSize: file.size, mimeType: file.type, dataUrl: dataUrl, duration: parsed.duration, importedAt: new Date().toISOString(), status: "loaded", role: "midi" }); setMidiStatus("loaded"); setMapping(undefined); } catch (caught) { setMidiStatus("error"); setMappingError(caught instanceof Error ? caught.message : "بارگذاری MIDI ناموفق بود."); } }; reader.onerror = () => { setMidiStatus("error"); setMappingError("خواندن فایل MIDI ناموفق بود."); }; reader.readAsDataURL(file); e.target.value = ""; }

  // 🧪 TEST: Import MIDI from data URL (DELETE AFTER TESTING)
  async function importMidiFromDataUrl(dataBase64: string, fileName: string) {
    const bytes = Uint8Array.from(atob(dataBase64), (c) => c.charCodeAt(0));
    const parsed = parseMidi(bytes.buffer, fileName, project.midiSettings.selectedTrackIndices, project.midiSettings.chordTolerance);
    setMidi(parsed);
    setMidiAsset({ id: crypto.randomUUID(), type: "midi", filePath: "", fileName, mimeType: "audio/midi", fileSize: bytes.length, dataUrl: `data:audio/midi;base64,${dataBase64}`, importedAt: new Date().toISOString(), status: "loaded", role: "midi" });
    setMidiStatus("loaded");
    setMapping(undefined);
  }

  // 🧪 TEST: Load preset by name from localStorage (DELETE AFTER TESTING)
  const loadFxPresetByName = useCallback((name: string) => {
    const raw = localStorage.getItem("piano-puzzle-custom-presets");
    if (!raw) return;
    const presets = JSON.parse(raw);
    if (presets[name]) setFxSettings((prev) => ({ ...prev, ...presets[name] } as VisualFxConfig));
  }, []);

  // 🧪 TEST: Button state (DELETE AFTER TESTING)
  ...
  const runTest = useCallback(async () => {
    ...
    try {
      await runTestLoad({
        acceptReferenceFrame,
        acceptPuzzleArtwork,
        importMidiFromDataUrl,
        setGeometryMode,
        generatePieces,
        runMapping,
        loadFxPreset: loadFxPresetByName,
        setPianoPlacement,
        setArtworkPlacement,
      }, setTestStatus);
    } finally {
  ...
  }, [testLoading, acceptReferenceFrame, acceptPuzzleArtwork, importMidiFromDataUrl, setGeometryMode, generatePieces, runMapping, loadFxPresetByName]);
  ...
  function handleCalibWheel(event: React.WheelEvent<HTMLDivElement>) { if (!calibration) return; zoomBy(event.deltaY < 0 ? 0.15 : -0.15); }
