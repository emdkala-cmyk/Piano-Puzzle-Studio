...
export function App() {
  ...
  useEffect(() => { referenceFrameRef.current = referenceFrame; geometryRef.current = geometry; mappingRef.current = mapping; timingRef.current = timingSettings; expressionRef.current = expressionSettings; showPieceBordersRef.current = showPieceBorders; audioEnabledRef.current = audioEnabled; }, [referenceFrame, geometry, mapping, timingSettings, expressionSettings, showPieceBorders, audioEnabled]);
  ...
  function rebuildPianoBackground(k: number) {
    ...
    if (refImage) {
      const placement = computeRegionPlacement(refImage.naturalWidth, refImage.naturalHeight, region);
