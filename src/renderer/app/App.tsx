...
export function App() {
  ...
  const spawnLineRef = useRef<HTMLDivElement>(null);
  ...
  function updateSpawnLineCSS() {
    if (!spawnLineRef.current) return;
    const frameH = puzzleHost.current?.clientHeight ?? 590;
    if (frameH <= 0) return;
    const pianoTop = pianoTopScreenYRef.current;
    const lineY = pianoTop - 8;
    spawnLineRef.current.style.top = Math.max(0, Math.min(99, (lineY / frameH) * 100)) + "%";
  }
  ...
  function applyPuzzleTransform() {
    ...
    if (fxRef.current) {
      ...
      if (spawnLineRef.current) {
        const screenY = offsetY + spawnLineY * k;
        const frameHeight = puzzleHost.current?.clientHeight ?? 590;
        spawnLineRef.current.style.top = ((screenY / frameHeight) * 100) + "%";
      }
    }
