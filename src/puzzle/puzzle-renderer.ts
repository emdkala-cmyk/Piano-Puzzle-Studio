...
export class PuzzleRenderer {
  ...
  rebuild(geometry: GeometryResult, scale = 1, texture?: Texture, placement?: RegionPlacement, showBorders = false) {
    this.layer.removeChildren(); this.views.clear();
    for (const piece of geometry.pieces) { const view = new PuzzlePieceView(piece, scale, texture, placement, showBorders); this.views.set(piece.id, view); this.layer.addChild(view.container); }
  }
