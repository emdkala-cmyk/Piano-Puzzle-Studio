...
export const DEFAULT_VISUAL_FX_CONFIG: VisualFxConfig = {
  ...
  smokeDensity: 0.06,
  smokeLayerCount: 2,
  smokeEmissionIntervalMs: 60,
  smokeFrameBudget: 20,
  particleFrameBudget: 200,
  bassThreshold: 48,
  highThreshold: 84,
  particleSize: 0.5,
  pathCurvature: 0.4,
  revealDurationMs: 400,
pathStyle: "scattered" as FxPathStyle,
...
pathStyle: ["sequential", "random", "spiral", "reverse", "scattered"].includes(merged.pathStyle as string) ? merged.pathStyle as FxPathStyle : "sequential",
    ...
    keyboardGlowThickness: Math.max(0.5, Math.min(20, Number.isFinite(merged.keyboardGlowThickness) ? merged.keyboardGlowThickness : 4)),
    keyboardGlowSpread: Math.max(5, Math.min(250, Number.isFinite(merged.keyboardGlowSpread) ? merged.keyboardGlowSpread : 60)),
