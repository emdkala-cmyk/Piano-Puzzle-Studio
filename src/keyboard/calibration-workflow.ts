import { createPianoLayout } from "./piano-layout";
...
import type { AnchorPoint, Calibration, Point, ViewType } from "./models";
...
export function createDefaultCalibration(width: number, height: number): Calibration {
  ...
  return { id: crypto.randomUUID(), name: "Keyboard Calibration", viewType: "top", keyboardType: "88-key", noteRange: { first: 21, last: 108 }, referenceWidth: width, referenceHeight: height, sourceWidth: width, sourceHeight: height, keyboardBounds: { x: 0, y: height * .12, width, height: height * .88 }, anchorPoints: anchorDefinitions.map(([id, kind, label, midiNote]) => ({ id, kind, label, midiNote, point: { x: width / 2, y: height / 2 } })), keyMap: createPianoLayout(), transform: identityTransform(), overlaySettings: overlayDefaults, cameraSettings: { liveCameraEnabled: false, referenceFrameTime: 0, opacity: 1, mirrorX: false, mirrorY: false, locked: false }, createdAt: now, updatedAt: now, version: "1.2.0" };
}
...
export function migrateCalibration(input: Calibration): Calibration {
  const next: Calibration = { ...createDefaultCalibration(input.sourceWidth || 1080, input.sourceHeight || 1920), ...input, version: "1.2.0", anchorPoints: input.anchorPoints?.length ? input.anchorPoints : createDefaultCalibration(input.sourceWidth || 1080, input.sourceHeight || 1920).anchorPoints };
